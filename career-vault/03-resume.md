# Resume Content

The resume is not the document; this file is. The PDF is just the formatted output. Keep the source content here, structured, easy to remix for different roles.

When you apply for a specific role, you copy from this file into a formatted template. Never write a resume from scratch for every application.

---

## Header

**Name:** *Your full name*
**Title:** *One line: your positioning sentence's role part. "UX Designer" or "Product Designer." Not "Designer."*
**Location:** *City, country*
**Contact:** *email, phone, link to portfolio*

---

## Profile (3-4 lines)

A condensed version of your positioning. The most important block on the resume because most recruiters read this and skip the rest.

> Example: "Product designer based in Dhaka. I work on trust-first mobile experiences for BD users on entry-level devices. Four months into my career, with three end-to-end projects shipped from confused brief to working HTML. I direct the work; Claude accelerates execution."

Three to four lines. Not a paragraph. Recruiters do not read paragraphs.

---

## Experience

Reverse chronological. For each role:

### [Role title]: [Company] *([dates])*

*One line summary of what you did there.*

- *Achievement, with a specific outcome or metric. Not "designed the app." Use the format: "[verb] [specific thing] [for/to specific result]."*
- *Example: "Rebuilt EduBridge booking flow for BD market. Mobile-first, bKash integration, document-verified tutor signal. Reduced time-to-first-booking by 40% in user testing."*
- *Example: "Established design tokens system for fintech MVP. 12 screens, three engineers, zero pixel disputes during handoff."*
- *Example: "Interrogated 8 client briefs before design started. Caught 23 contradictions that would have caused mid-sprint rework."*

Three to five bullets per role. Bullets are decisions you made, not tasks you completed.

---

## Selected projects

If your experience section is thin (junior career), list 2-3 projects here with one-line summaries each. Link to the corresponding portfolio story in `02-portfolio-story.md`.

- **EduBridge BD: Tutor booking flow** *(course project, 2026)*
  - Parent-first mobile flow with document-verified trust signals, bKash payment, Bangla + English support. Built end-to-end including HTML.

- *Your project 2*

- *Your project 3*

---

## Skills

Group by category. Be specific. "Figma" is fine if you can prove competence. "Photoshop" is filler if it's not core to your work: remove.

### Design
- *Example: Mobile-first product design, design systems, IA, interaction design*

### Tools
- *Example: Figma, Claude (Chat, Code, Cowork), VS Code*

### Specialized
- *Example: BD market design (low-end Android, slow networks, bKash), bilingual UI (Bangla + English), token-based design systems*

---

## Education

Reverse chronological. Include relevant only.

- *[Degree]: [Institution] ([year])*
- *Claude for UI/UX Designers: Ostad ([year])*

---

## Things I will NOT put on this resume

The discipline part. Some things look impressive but signal weakness:

- "Proficient in 47 tools." Recruiters know nobody is proficient in 47 tools. Pick 5.
- "Team player." Empty. Replace with a specific instance of teamwork in a bullet.
- "Detail-oriented." Same.
- Self-rated skill bars (Figma: 4/5 stars). Reads as juvenile in 2026.
- Photo. BD norms differ. Default to no photo unless the role explicitly asks.
- Personal interests unless they tie to your work (e.g., "I redesign BD government forms for fun": yes; "I love hiking": no).

---

## Resume variants

Keep separate sections of this file per role type:

### Variant: Product Design roles
*Profile + experience emphasizing IA, user research, system thinking*

### Variant: Frontend-leaning roles
*Profile + experience emphasizing Claude Code, HTML/CSS familiarity, design-engineering handoff*

### Variant: Agency or consulting roles
*Profile + experience emphasizing brief interrogation, client communication, multiple project ownership*

When you apply, you pick the variant whose profile fits the role, and use the experience bullets that match.
